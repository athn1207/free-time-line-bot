...
- **方法 A（Render の Shell で実行）**  
  1. Render ダッシュボードで **free-time-line-bot（Web サービス）** を開く。  
  2. 左メニュー **Shell** をクリックし、ブラウザでシェルを開く。  
  3. 先に **Environment** で **DATABASE_URL** を追加・保存し、**再デプロイ**しておく（Shell では既存の環境変数が使われます）。  
  4. Shell で `node scripts/init-db.js` を実行する。  
  「テーブル作成完了: users, oauth_states」と出れば OK。

- **方法 B（ローカルで実行）**  
  1. ローカルの `.env` に、手順 2 でコピーした **DATABASE_URL** を 1 行で追加する。  
  2. ターミナルでプロジェクトフォルダに移動し、`node scripts/init-db.js` を実行する。  
  3. 同じく「テーブル作成完了」と出れば OK（その後、.env の DATABASE_URL は本番用なので削除してよい）。
